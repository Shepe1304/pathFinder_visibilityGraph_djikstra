...
def drawGrid():
    ...
    for i in range(0, WIDTH + 1000, cellSize):
        canvas.create_line([(i - offset, 0 - offset), (i - offset, HEIGHT + 1000 - offset)], tag='grid_line',
                           fill="lightgray")
    for i in range(0, HEIGHT + 1000 * offset, cellSize):
        canvas.create_line([(0 - offset, i - offset), (WIDTH + 1000 - offset, i - offset)], tag='grid_line',
                           fill="lightgray")
...
def drawMap(pointList, edgeList):
    for pair in edgeList:
        ...
        canvas.create_line([(point1[0] - offset, point1[1] - offset), (point2[0] - offset, point2[1] - offset)],
                           tag='map_line', fill="black")
    for point in pointList:
    ...
    if last_repetitions >= repetitions:
        ...
        for i in range(len(resConnections)):
            for x in resConnections[i]:
                canvas.create_line(
                    [(currentTranslatedPointList[i][0] - offset, currentTranslatedPointList[i][1] - offset),
                     (currentTranslatedPointList[x][0] - offset, currentTranslatedPointList[x][1] - offset)],
...
def showInfo():
    ...
    visLabel3 = Label(line_group, text=f"Visibility Lines: {visLines}", font='Helvetica 13', fg="red", padx=40).pack(
        anchor=W)
    visLabel4 = Label(line_group, text=f"Shortest Distance: {shortest_distance_display}", font='Helvetica 13', fg="red",
                      padx=40).pack(anchor=W)
    visLabel4 = Label(line_group, text=f"Run Time: {runtimeDisplay}", font='Helvetica 13', fg="red", padx=40).pack(
        anchor=W)
...
def drawVisibilityGraphStartGoal(mode):
    ...
    for i in range(len(currentVisibilityGraphPointList) - 2):
        ...
        if len(currentVisibilityGraphPointList) >= len(currentPointList):
            for pair in currentEdgeList:
                if not visible(detranslated_pt, currentVisibilityGraphPointList[i],
        ...
        if connect:
            ...
            canvas.create_line([pt, (currentTranslatedVisibilityGraphPointList[i][0] - offset,
...
def shortestPath(code):
    ...
    if int(entryRepeat.get()) == 0:
        ...
        if start_point != (-10, -10) and goal_point != (-10, -10):
            canvas.create_line([start_point, goal_point],
    ...
    if start_goal_connect:
        canvas.create_line([start_point, goal_point], tag="shortest_path",
                           fill="blue", width=3)
    else:
        canvas.create_line([start_point, (currentTranslatedPointList[shortest_path[1]][0] - offset,
                                          currentTranslatedPointList[shortest_path[1]][1] - offset)],
                           tag="shortest_path", fill="blue", width=3)
    ...
    if not start_goal_connect:
        canvas.create_line([(currentTranslatedPointList[shortest_path[len(shortest_path) - 2]][0] - offset,
                            ...
                            goal_point], tag="shortest_path", fill="blue", width=3)
...
line_group.pack(anchor=W)
Button(control_panel, text="GENERATE", font='Helvetica 14 bold', bd=5, fg="black",
       command=lambda: generateMap(pointList, edgeList, int(entryRepeat.get()), 'none', True)).pack()
Label(control_panel, text="").pack()
Label(control_panel, text="CHOOSE POINTS", font='Helvetica 14 bold', fg="darkred", pady=10).pack()
line_group = Label(control_panel, text="")
Button(line_group, text="Start", font='Helvetica 14 bold', bd=5, fg="darkgreen", command=chooseStartPoint).grid(row=0,
                                                                                                                column=0)
Label(line_group, text=" ").grid(row=0, column=1)
Button(line_group, text="Goal", font='Helvetica 14 bold', bd=5, fg="orange", command=chooseGoalPoint).grid(row=0,
                                                                                                           column=2)
line_group.pack()

Label(visibility_panel, text="VISIBILITY GRAPH", font='Helvetica 16 bold underline', pady=10).grid(row=0, column=0)
Button(visibility_panel, text="GENERATE", font='Helvetica 14 bold', bd=5, fg="black",
       command=lambda: visibilityGraph(pointList, edgeList, connections, int(entryRepeat.get()), True)).grid(row=1,
                                                                                                             column=0)

Label(visibility_panel, text=" ", font="Helvetica 2").grid(row=2, column=0)
Button(visibility_panel, text="SHOW INFORMATION", font='Helvetica 16 bold', bd=5, command=showInfo, bg="lightgreen",
       fg="darkgreen").grid(row=3, column=0)

Label(root, text="     ").grid(row=1, column=1)

lastButtonLine = Label(root)
findPath = Button(lastButtonLine, text="Shortest Path", command=lambda: shortestPath(1), font='Helvetica 20 bold', bd=5,
                  fg="darkblue", padx=30)
clearButton = Button(lastButtonLine, text="Clear", command=lambda: clearCanvas(1), font='Helvetica 20 bold', bd=5,
                     fg="darkred", padx=30)

canvas.create_rectangle(canvasBorderWidth, canvasBorderWidth, WIDTH - canvasBorderWidth + 4,
                        HEIGHT - canvasBorderWidth + 4, outline='lightgray', width=4)
